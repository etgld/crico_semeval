    ...
    if target_entities:
        for source_entity in data.annotations.select_type(source_type):
            ...
            def distance_to_source_entity(entity):
                return min(
                    abs(p1 - p2)
                    for p1 in points[source_entity.id]
            ...
            relation.id = "{0}@{1}@{2}".format(
                source_entity.id, relation_type, target_entity.id
            )
...
if __name__ == "__main__":
    ...
    relations_to_closest_parser = subparsers.add_parser(
        "closest",
        help="Link each source annotation to the target "
