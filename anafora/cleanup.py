    ...
    for sub_dir, text_name, xml_names in anafora.walk(input_dir, xml_name_regex):
        for xml_name in xml_names:
            ...
            for (
                span,
                annotations,
            ) in anafora.validate.find_entities_with_identical_spans(data):
                try:
                    ...
                    if (
                        special_time.type in {"SECTIONTIME", "DOCTIME"}
                        and timex.type == "TIMEX3"
                    ):
...
def convert_thyme_qa_to_anafora_xml(input_dir, output_dir):
    ...
    _text_clip_pattern = regex.compile(
        r"Text Clip:\s+\d+[.]\s+(\d+),(\d+) (Exact|Support)_Answer "
            ...
            if input_file_name.endswith("_qa.txt"):
                ...
                with open(os.path.join(input_root, input_file_name)) as input_file:
                    ...
                    for annotation_text in _annotation_sep_pattern.split(
                        ...
                        if match is None:
                            raise ValueError(
                                "Invalid annotation text: " + annotation_text
                            )
                        (
                            question,
                            answer,
                        ...
                        if len(text_clip_text.splitlines()) != 2 * len(
                            text_clip_matches
                        ):
                            raise ValueError(
                        ...
                        for (
                            ...
                            end_text,
                            _,
                            time_or_doc_time_rel,
                            type_text,
                            clip_text,
                        ) in text_clip_matches:
                            begin = int(begin_text)
                            end = int(end_text)
                            entity = anafora.AnaforaEntity()
                            entity.id = "{0:d}@{1}@{2}@gold".format(
                                count, "e", file_base
                            )
...
if __name__ == "__main__":
    ...
    thyme_parser.add_argument(
        "-s",
        "--schema",
        metavar="FILE",
        required=True,
        type=anafora.validate.Schema.from_file,
